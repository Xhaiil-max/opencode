...
function ParticipantsTab({
  ...
  const HostControlsMenu = ({ isPinned, onClose, onAction, onTogglePin, onUnpin, buttonEl }: {
    ...
    return (
      <div
        className="fixed z-[9999] glass-strong rounded-xl p-1 shadow-2xl min-w-[160px] animate-slide-up"
        style={position}
      >
...
function ToolsTab({
  ...
  const createPoll = () => {
    ...
    // Trigger a custom event that the chat can pick up
    window.dispatchEvent(new CustomEvent('sendChatMessage', { detail: pollText }))
    setPollQuestion('')
    setPollOptions(['', ''])
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-thin">
      ...
      {/* Poll Widget */}
      <div className="p-3 bg-bg-tertiary border border-border-primary rounded-xl">
        <h4 className="text-xs font-medium text-text-muted mb-3 uppercase tracking-wide">Poll</h4>
        {showPollCreator ? (
            ...
            {pollOptions.map((opt, i) => (
              <div key={i} className="flex items-center gap-1">
                ...
                {pollOptions.length > 2 && (
                  <button onClick={() => setPollOptions(pollOptions.filter((_, j) => j !== i))} className="p-1 text-text-muted hover:text-accent-error cursor-pointer">
                    <X size={12} />
                  </button>
                )}
              </div>
            ))}
            <button onClick={() => setPollOptions([...pollOptions, ''])} className="text-xs text-haze-400 hover:text-haze-300 cursor-pointer">+ Add option</button>
            <div className="flex gap-2 pt-1">
              <button onClick={createPoll} className="btn-primary text-xs px-3 py-1.5 cursor-pointer">Send Poll</button>
              <button onClick={() => setShowPollCreator(false)} className="text-xs text-text-muted hover:text-text-primary cursor-pointer">Cancel</button>
            </div>
          </div>
        ) : (
          <button onClick={() => setShowPollCreator(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg bg-bg-secondary border border-border-primary hover:bg-bg-tertiary transition-colors cursor-pointer">
            <span className="text-lg">📋</span>
            <div className="flex-1 text-left">
              <div className="text-sm font-medium text-text-primary">Create Poll</div>
              <div className="text-xs text-text-muted">Send a poll to the chat</div>
            </div>
          </button>
        )}
      </div>
      ...
      <div className="p-3 bg-bg-tertiary border border-border-primary rounded-xl">
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-xs font-medium text-text-muted uppercase tracking-wide">Notes</h4>
          {notesContent && (
            <button onClick={downloadNotes} className="text-xs text-haze-400 hover:text-haze-300 flex items-center gap-1 cursor-pointer">
        ...
        <textarea
          ...
          className="input w-full resize-none text-sm min-h-[100px]"
          rows={6}
        />
      </div>
    </div>
